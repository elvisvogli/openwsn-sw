...
implementation {
   ...
   timervalue_t      periodADV = 10000;//poipoi
   ...
   task void taskBuildAndSendAdv() {
      ...
      call timerADV.startPeriodic((call Random.rand32())%(periodADV*2));
   }
   ...
   event void timerADV.fired() {
      ...
      if (temp_busyADV==TRUE){
         //this is not an error as it can happen when the ADV message has not been sent yet
         return;
      }
      atomic busyADV = TRUE;
